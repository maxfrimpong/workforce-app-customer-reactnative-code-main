...
export const Chat = ({ navigation, route }) => {
    ...
    useEffect(() => {
        dispatch(getChatStart({ driverId: chatScreenData.driverId }))
            ...
            <FlatList
                ...
                renderItem={({ item, index }) =>
                    <View style={[styles.itemWrapper, {
                        alignItems: item?.byCustomer ? 'flex-end' : 'flex-start'
                    }]}>
                        <TouchableOpacity
                            activeOpacity={0.7}
                            onPress={handleItemPress(item)}
                            style={styles.bubbleWrapper}>
                            {item?.byCustomer && <Image
                                style={styles.chatProfile}
                                source={user?.profileImage ? { uri: user?.profileImage } : images.dummyProfile}
                            />}
                            <View style={[styles.bubbleStyle, {
                                backgroundColor: item?.byCustomer ? colors.primary : colors.inputBackground
                            }]}>
                                {item?.msg
                                    ? <Text style={[styles.msgText, {
                                        color: item?.byCustomer ? colors.white : colors.textBlack
                                    }]}>
                                        {item?.msg}
                                    </Text>
                                    : <Image
                        ...
                        <Text style={[styles.timeText, {
                            marginLeft: item?.byCustomer ? '13%' : 0
                        }]}>
