...
import {currentRouteName, navigate} from '../../navigation/RootNavigation';
...
export const NotificationHandler = () => {
  ...
  const onComponentMount = async () => {
    ...
    messaging().onMessage(async remoteMessage => {
      ...
      if (
        currentRouteName() != screenName.chat ||
        remoteMessage?.data?.type != 'chat'
      ) {
        handleDisplayNotifee(remoteMessage?.notification, remoteMessage?.data);
      }
    });
  ...
  const checkPermission = async () => {
    ...
    if (authorizationStatus === 1) {
      ...
      errorToast(
        'Notification permissions are not allowed. Please update Workforce notification permissions from settings',
      );
  ...
  const handleDisplayNotifee = async (notification, data) => {
    ...
    try {
      ...
      if (Platform.OS === 'android') {
        showInAppNotification(notification, data, notificationId);
      }
    } catch (e) {
  ...
  const notificationMapper = {
    bookingApproved: data =>
      handleNavigation(screenName.tracking, {tripId: data?.tripId}),
    bidPlaced: data =>
      handleNavigation(screenName.providers, {
        item: {_id: data?.tripId, tripStatus: data?.type},
      }),
    jobMatched: data =>
      handleNavigation(screenName.tracking, {tripId: data?.tripId}),
    chat: data => handleNavigation(screenName.chat, {driverId: data?.driverId}),
  };
    ...
    showMessage({
      ...
      renderCustomContent: () => (
        <View style={styles.customContentWrapper}>
          <Image style={styles.appIcon} source={images.icLogo} />
